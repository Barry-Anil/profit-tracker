...
const StageDetailTable = ({ orderData, rowID, category }: { orderData: any; rowID: any; category: any }) => {
    ...
    const columns: ColumnDef<any>[] = [
            ...
            header: ({ column }) => {
                return (
                    <Button variant="ghost" onClick={() => column.toggleSorting()}>
                        Order Date <span className="text-primary ml-1"> {column.getIsSorted() === 'asc' ? '(Asc)' : column.getIsSorted() === 'desc' ? '(Desc)' : ''}</span>
        ...
        },
        {
            ...
            header: ({ column }) => {
                return (
                    <Button variant="ghost" onClick={() => column.toggleSorting()}>
                        Order Since <span className="text-primary ml-1"> {column.getIsSorted() === 'asc' ? '(Desc)' : column.getIsSorted() === 'desc' ? '(Asc)' : ''}</span>
                        <ArrowUpDown className="ml-2 h-4 w-4" />
                    </Button>
                );
            },
            cell: ({ row }) => {
                const formatted_date = new Date(String(row?.original.orderdate)?.slice(0, 24));
                const today_date = new Date();
                const order_since = Math.ceil((today_date.getTime() - formatted_date.getTime()) / (1000 * 60 * 60 * 24));

                return <p>{order_since}</p>;
            },
        },
        {
    ...
    return (
        <div className="space-y-2">
            <p className="text-lg font-semibold text-red-500">Invoice Amount Total(INT): 0</p>
