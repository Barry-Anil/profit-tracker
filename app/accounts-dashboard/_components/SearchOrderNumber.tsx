...
const SearchOrderNumber = ({ userDataDetails, amountReceived }: { userDataDetails: any; amountReceived: any }) => {
    ...
    const handleClick = async () => {
        userDataDetails.refetch();
    };

    let amount_received =
        amountReceived?.data === undefined
            ? 0
            : Number(
                  amountReceived?.data?.data
                      ?.filter((item: any) => item.accounts_payment_approval === 'PP ')
                      .reduce((acc: any, item: any) => acc + Number.parseFloat(item.accounts_invoiceamt_currencyint), 0)
                      .toFixed(2)
              ) +
              Number(
                  amountReceived?.data?.data
                      .filter((item: any) => item.accounts_payment_approval === 'FP ')
                      .reduce((acc: any, item: any) => acc + Number.parseFloat(item.accounts_invoiceamt_currencyint), 0)
                      .toFixed(2)
              );
    ...
    return (
        <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="space-y-1">
                <p className="font-medium flex items-center gap-1">Search Order Number</p>
            ...
            <div className="flex flex-wrap items-center gap-4">
                <div className="space-y-1">
                    ...
                    <Input type="text" name="amountRecd" placeholder="Amount Recd" disabled value={amount_received} />
                </div>
                <div className="space-y-1">
                    <p className="font-medium">Pending Payment</p>
                    <Input type="text" name="pendingPayment" placeholder="Pending Payment" disabled value={pending_payment} />
                </div>
            </div>
        </div>
    );
